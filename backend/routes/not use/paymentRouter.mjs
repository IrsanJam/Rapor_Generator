  ...
  try {
    ...
    snap
      ...
      .then(async (transaction) => {
        ...
        const newTransaction = await paymentModel.create({
          dataSemua: resultData,
          orderId: userId,
        });
...
router.put('/payment/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const updatedOrder = await paymentModel.findOneAndUpdate(
      { 'dataSemua._id': id },
      { $set: { 'dataSemua.$.status': req.body.status } },
      { new: true }
    );
...
router.post('/payment-notifications', async (req, res) => {
  ...
  const payments = await paymentModel.find({
    'dataSemua.transaction_id': req.body.order_id,
    'dataSemua.transaction_status': 'settlement',
  });
